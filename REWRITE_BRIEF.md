...
### 2차 수정 완료 기록 (2026-08-17)

미발행 28건 전부 **오류 0 · 경고 0**, `crossDraftRepeats` **출력 없음**
(limit 을 2 로 낮춰도 걸리는 상투구가 없다).
발행 완료된 12건은 손대지 않았다.

무엇을 했나
- `정리하면` 마무리 25건 제거. 10건은 여는 줄을 아예 없애고 요약 불릿으로 바로 들어갔고,
  15건은 그 글의 내용에서 나온 한 줄로 열었다
  (`토한 색이 방향을 정해줍니다` · `10kg 한 줄이 전부를 가릅니다` ·
   `지우는 싸움에서 말리는 싸움으로 옮긴 게 전부예요` 등 — 28건이 전부 다른 문장이다).
- `아직 남은 애로` 7건을 그 글의 말로 풀었다
  (`걸리는 건 더위예요` / `그래도 에너지가 남는 날은 있어요` / `불편한 것도 있어요` …).
- 제목 23건에 클릭 장치를 넣었다. 장치를 섞었다 —
  말줄임표 4 · 질문형 4 · 반전 6 · 비교 4 · 금지·실수·피하 5.
  **검색어는 앞쪽에 그대로 뒀다**(강아지 구토 / 중형견 한 달 유지비 / 일본 오사카 반려견 동반 …).
- 시점 표기 규칙 변경 반영. timeless 8건에서 날짜를 뺐다 —
  허리 디스크 · 검은 발톱 · 줄당김 · 사료 기준 · 구토 · 분리불안 · 슬개골 · 짐 싸기.
  남긴 건 형식을 섞었다 — `(2026.8)` 다수, `(2026년 기준)`(유지비), `(2026)`(물그릇).
  ※함정: `TIME_NEEDED` 정규식에 `법` 이 있어서 `~하는 법` 이 들어간 제목은
    timeless 인데도 날짜를 요구한다. 그래서 그 8건은 제목에서 `법` 을 뺐다.
- 예고형으로 닫히던 도입부·소제목 5건 수정. `짚어볼게요` `확인해봤어요`
  `정리해볼게요` 로 닫던 자리를 답을 미루는 문장으로 바꿨고,
  짐싸기·맡기기·슬개골은 검색어 리드 문단을 `망고아빠입니다` 아래로 내려
  나머지 25건과 구조를 맞췄다.
- `그 뒤로 순서를 바꿨어요` 2건 제거.
  보홀 글은 이 김에 취소 수수료 일화의 출처도 바로잡았다 —
  `세부에서 오션뷰 방을 덜컥 잡았다가` 는 그 도시에서 겪은 일처럼 읽혀서
  `예전에 일본에서 … 렌터카 체중 제한에 걸려` 로 고쳤다(원 사건은 일본 한 번뿐이다).

지어내지 않았다
- 새 일화를 만들지 않았다. 모든 수정은 이미 본문에 있던 사건·수치를 다른 문장으로 옮긴 것이다.
- 본문 중간의 사실·수치·표는 한 글자도 건드리지 않았다.

교훈 하나 더
`정리하면` 을 없앨 때 25건에 같은 대체 문구를 넣으면 제자리다.
그래서 고치는 중간중간 `crossDraftRepeats` 를 돌렸고, 마지막엔 limit 2 로도 확인했다.
틀을 없애는 작업은 반드시 전체를 가로질러 세면서 해야 한다.
