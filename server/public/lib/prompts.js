...
이 글은 광고가 붙는 **최종 목적지 글**이야 — 가장 깊이 있고 완성도 높아야 해.
...
export function buildCushionPrompt(platform, { sourceText, bloggerUrl, keyword, audience, tone, authorBio, today, imageCount, reference }) {
  const resolved = resolveType("auto", keyword || "");
  const imgN = Math.min(6, Math.max(1, imageCount || 2));
  const isNaver = platform !== "wp";
  const where = isNaver ? "네이버 블로그" : "워드프레스";

  // 플랫폼별 정체성/링크 전략
  const identity = isNaver
    ? `[정체성] 네이버 블로그용 **유입 쿠션 글**. 친근한 구어체, 짧은 문단, 이모지 적절히, 독자에게 말 걸듯 편하게.`
    : `[정체성] 워드프레스용 **하이브리드 글(반은 쿠션·반은 최종목적지)**. 그 자체로 읽을 가치가 있는 깊이 있는 정보성 글이면서, 동시에 더 자세한 원본으로 유입도 시킨다. 톤은 정돈되고 신뢰감 있는 정보성(과한 구어체 X).`;

  const linkRule = isNaver
    ? `[링크 규칙 — 네이버는 엄격]
- 네이버는 외부 블로그 링크에 불이익이 크다. 그래서 **외부 링크는 딱 1개만** 넣어라: 글 후반부에 "▶ 자세한 내용 전체 보기" 형태의 cta(또는 linkcard 1개) **하나**로 목적지(원본) 연결. 그 외 넷플릭스·유튜브·타 블로그 등 외부 링크는 넣지 마라.
- 상단에 낚시성 버튼 카드 여러 개 금지. 대신 도입부는 궁금증만 유도하고, 본문으로 충분히 정보를 준 뒤 마지막에 자연스럽게 그 1개 링크로 유도.`
    : `[링크 규칙 — 워드프레스 하이브리드]
- 상단(도입부 직후)에 linkcard 1개로 핵심 클릭 유도 버튼 2~3개(예: "전체 내용 자세히 보기", "관련 정보 총정리"). 본문 곳곳에 cta 1~2개.
- 링크의 절반 정도는 "자세히 보기/전체 내용 보러가기"(→ 목적지·원본 연결), 나머지 절반은 공식 출처(넷플릭스·유튜브·나무위키 등) 또는 내 다른 글(연관글)로 연결. 쿠션끼리 상호 링크 환영.
